...
# 提取规则计数
num_rules = subprocess.getoutput("sed -n 's/^! Total count: //p' rules.txt")
num_dns = subprocess.getoutput("sed -n 's/^! Total count: //p' dns.txt")
num_allow = subprocess.getoutput("sed -n 's/^! Total count: //p' allow.txt")
...
subprocess.run(f"sed -i 's/^拦截规则数量.*/拦截规则数量: {num_rules} /g' README.md", shell=True)
